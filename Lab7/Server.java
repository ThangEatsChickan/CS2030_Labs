...
class Server {
	...
	public Server(Server s) {
		...
		this.waitingCustomer = s.waitingCustomer;
		this.id = s.id;
	}
	...
	public Server makeIdle() {
		...
		server.currentCustomer = Optional.empty();
		return server;
	}
	...
	public boolean isIdle() {
		Server server = new Server(this);
		return server.currentCustomer.isEmpty();
	}
	...
	public boolean hasWaitingCustomer() {
		Server server = new Server(this);
		return server.waitingCustomer.isPresent();
	}
	...
	public Optional<Customer> getWaitingCustomer() {
		Server server = new Server(this);
		Optional<Customer> cust = server.waitingCustomer;
		return cust;
	}
  ...
  public Server serve(Customer customer) {
	Server server = new Server(this);
	server.currentCustomer = Optional.of(customer);
	if(customer.equals(server.currentCustomer.orElse(new Customer(0, -1)))) {
	  server.waitingCustomer = Optional.empty();
	}
    return server;
  }
	 ...
	 */
	public Server askToWait(Customer customer) { //was just server previously
		Server server = new Server(this);
		server.waitingCustomer = Optional.of(customer);
		return server;
	}
